...
TEST_CASE("hopfield rejects patterns with wrong size", "[hopfield][errors]")
{
    ...
    REQUIRE_THROWS_AS(network.learn({{1.0F, -1.0F, 1.0F}}), std::runtime_error);
...
TEST_CASE("hopfield learning keeps diagonal zero and weights symmetric", "[hopfield]")
{
    ...
    for (size_t i = 0; i < layer->weights.size(); ++i) {
        REQUIRE(layer->weights[i][i] == 0.0F);
        for (size_t j = 0; j < layer->weights[i].size(); ++j) {
            REQUIRE(layer->weights[i][j] == layer->weights[j][i]);
        }
    }
}
